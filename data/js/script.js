...
function goToHome() {
    ...
    if (statsCard && homeSelect && homeSelect.value !== "") {
...
function executeGeneralSearch(query, secondQuery = null) {
    ...
    let dbPath = `voters/${union}/ward_${ward}`;
...
function updateLiveCount() {
    ...
    if ((ward === "3" || ward === "5" || ward === "7") && subArea === "") {
    ...
    let dbPath = `voters/${union}/ward_${ward}`;
    if (subArea !== "সাধারণ" && subArea !== "") dbPath += `/${subArea}`;
...
async function loadHomeStats() {
    ...
    try {
        ...
        snapshots.forEach((snapshot, index) => {
            ...
            wardBox.innerHTML = `<span style="font-size: 12px; color: #666;">ওয়ার্ড ${enToBn(wardNum.toString())}</span><b style="color: #004529; font-size: 15px;">${enToBn(finalWardTotal.toString())} জন</b>${subAreaDetailsHTML}`;
    ...
    } catch (error) { loader.innerHTML = "<span style='color:red;'>তথ্য লোড করতে সমস্যা হয়েছে!</span>"; }
}
...
function exitApp() { if (confirm("আপনি কি অ্যাপ থেকে বের হয়ে যেতে চান?")) window.location.href = "about:blank"; }
