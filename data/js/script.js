...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // ২. ডিফল্ট ইউনিয়ন পরিসংখ্যান লোড করা
    const homeSelect = document.getElementById('home-union-select');
    if(homeSelect) {
    ...
    // ৩. আপনার (ওনার) জন্য বিশেষ সুবিধা: অ্যাড হাইড করা
    if (localStorage.getItem('admin_mode') === 'owner_access_true') {
        // পপ-আপ অ্যাড মোডালটি চিরতরে রিমুভ করা
        const adModal = document.getElementById('ad-modal');
        if (adModal) adModal.remove();

        // পেজের অন্যান্য ফিক্সড অ্যাড কন্টেইনারগুলো হাইড করা
        const adContainers = document.querySelectorAll('[id*="container-"], [id*="adsterra-box"]');
        adContainers.forEach(container => {
            container.style.display = 'none';
        });

        console.log("Admin mode active: Your device is now ad-free.");
    }
});

/**
 * ৪. ওনার (মাসুদ রানা) জন্য গোপন অ্যাডমিন সুইচ
 * ফুটারের "মোহাম্মদ মাসুদ রানা" নামের ওপর ৭ বার ক্লিক করলে অ্যাড বন্ধ হবে
 */
let devClickCount = 0;
document.addEventListener('click', function(e) {
    if (e.target.innerText && e.target.innerText.includes('মোহাম্মদ মাসুদ রানা')) {
        devClickCount++;
        if (devClickCount === 7) {
            localStorage.setItem('admin_mode', 'owner_access_true');
            alert('সফল হয়েছে! মাসুদ ভাই, আপনার ব্রাউজারে এখন থেকে আর অ্যাড দেখাবে না।');
            location.reload(); // সেটিংস অ্যাপ্লাই করার জন্য পেজ রিফ্রেশ
        }
    }
});
...
// ১. অ্যাড দেখানোর মেইন ফাংশন
function showAd() {
    // যদি আপনি অ্যাডমিন মোডে থাকেন তবে অ্যাড দেখাবে না
    if (localStorage.getItem('admin_mode') === 'owner_access_true') return;

    const adModal = document.getElementById('ad-modal');
    const closeBtn = document.getElementById('close-ad-btn');
    const timerText = document.getElementById('ad-timer-text');
    const secSpan = document.getElementById('ad-sec');
    
    // রিসেট UI (আগের বার বন্ধ করা থাকলে সেটি ঠিক করা)
    ...
    let timeLeft = 15; // আপনার টাইমার 15 সেকেন্ড
    ...
    const timer = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            ...
            if (closeBtn) closeBtn.style.display = 'inline-block'; // 15 সেকেন্ড পর বাটন আসবে
        }
    }, 1500);
}

// ২. অ্যাড বন্ধ করার ফাংশন
function closeAd() {
    const adModal = document.getElementById('ad-modal');
    if (adModal) {
    ...
    // বর্তমান সময় সেভ করে রাখা যাতে ২০ মিনিট আগে আর অ্যাড না আসে
    localStorage.setItem('last_ad_timestamp', new Date().getTime());
}

// ৩. অ্যাড দেখানোর টাইম চেক (২০ মিনিট পর পর)
...
// ৪. পেজ লোড হওয়ার ৫ সেকেন্ড পর প্রথমবার অ্যাড চেক করবে
setTimeout(checkAdStatus, 5000);

// ৫. প্রতি ২ মিনিট পর পর ব্যাকগ্রাউন্ডে চেক করবে ২০ মিনিট পার হয়েছে কি না
setInterval(checkAdStatus, 120000); 
